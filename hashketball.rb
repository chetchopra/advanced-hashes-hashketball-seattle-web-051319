...
def team_names 
  return teamNames = [game_hash[:home][:team_name], game_hash[:away][:team_name]]
end

def player_numbers(teamName)
  team = game_hash[:home][:team_name] === teamName ? game_hash[:home][:players] :  game_hash[:away][:players]
  playerNums = []
  team.each do |player, value|
    playerNums << value[:number]
  end
  return playerNums
end
...
def big_shoe_rebounds
  ...
  players.each do |player, value|
    if value[:shoe] > biggestShoe
      biggestShoe = value[:shoe]
